...
import { 
    Search, FileText, Globe, School, ChevronLeft, ChevronRight, Loader2, Filter, X, Eye
} from "lucide-react";
                ...
                <Table>
                    <TableHeader className="bg-slate-50/50">
                        <TableRow className="border-slate-100">
                            ...
                            <TableHead className="text-[9px] font-black uppercase text-slate-400 h-12 text-center">Actions</TableHead>
                            <TableHead className="text-[9px] font-black uppercase text-slate-400 pr-8 text-right h-12">Status</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {isLoading ? (
                                ...
                                <TableCell>
                                    <div className="flex flex-col">
                                        <div className="flex items-end gap-1 mb-1">
                                            <span className="text-sm font-black text-slate-900 leading-none">{s.totalScore}</span>
                                            <span className="text-[9px] text-slate-300 font-bold uppercase">/ {s.maxScore}</span>
                                        </div>
                                        <Badge className={cn("text-[8px] font-black uppercase h-4 px-1.5 border-none", s.confidence > 0.8 ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600")}>
                                ...
                                <TableCell>
                                    <div className="flex flex-col gap-1">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate max-w-[150px]">{s.schoolName}</span>
                                        </div>
                                        <div className="flex items-center gap-1.5">
                                            <Globe className="h-3 w-3 text-slate-300" />
                                            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{s.machineId}</span>
                                        </div>
                                    </div>
                                </TableCell>
                                <TableCell className="text-center">
                                    <Button variant="ghost" className="h-8 w-8 p-0 rounded-lg hover:bg-indigo-50 text-indigo-600">
                                        <Eye className="h-4 w-4" />
                                    </Button>
                                </TableCell>
                                <TableCell className="pr-8 text-right">
                                    {s.reviewRequired ? (
                                        <Badge className="bg-amber-50 text-amber-600 border-none text-[9px] font-black uppercase h-5 px-2">QA Review</Badge>
                                    ) : (
